// Package cli assembles the trivia command tree from the trivia
...
// NewApp assembles the kit application from the trivia domain. The
...
//
// To add a command, declare it in trivia/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := trivia.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(trivia.Domain{}).Register(app)
