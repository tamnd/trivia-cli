...
describes what users get. Pick a format with `-o`, or let trivia choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
trivia <command> -o table   # aligned columns for reading
trivia <command> -o jsonl   # one JSON object per line, for piping
trivia <command> -o json    # a single JSON array
trivia <command> -o csv     # spreadsheet friendly
trivia <command> -o tsv     # tab-separated
trivia <command> -o url     # just the URL column
trivia <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
trivia <command> --fields id,title,url
```
...
```bash
trivia <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
trivia <command>            # a table, because this is a terminal
trivia <command> | wc -l    # JSONL, because this is a pipe
```
